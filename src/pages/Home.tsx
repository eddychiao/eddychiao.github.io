...
const DESC   = "I'm a software developer currently based in New York City. I love building all different kinds of cool and fun things with code. Feel free to explore my website to see what I've made!";
...
const Home: React.FC<HomeProps> = ({ theme }) => {
  ...
  const { typed: descText,  done: descDone  } = useTypingAnimation(DESC, 15, titleDone ? 320 : 999999);
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (cardRef.current) {
      cardRef.current.style.transition = 'transform 0.08s ease, box-shadow 0.08s ease';
      cardRef.current.style.transform  =
        `perspective(900px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) scale(1.04)`;
      cardRef.current.style.boxShadow  =
        `${-tiltY * 2}px ${tiltX * 2}px 50px ${theme.buttonColor}55`;
    }

    if (shineRef.current) {
      shineRef.current.style.opacity    = '1';
      shineRef.current.style.background =
        `radial-gradient(circle at ${x * 100}% ${y * 100}%, rgba(255,255,255,0.18) 0%, transparent 62%)`;
    }
  };
  ...
  const handleMouseLeave = () => {
    ...
    if (cardRef.current) {
      cardRef.current.style.transition = 'transform 0.65s ease, box-shadow 0.65s ease';
      cardRef.current.style.transform  =
  ...
  return (
    <div className="Home" style={{ backgroundColor: theme.backgroundColor }}>
      <div className="Home-left">
        ...
        <div className="Home-title" style={{ color: theme.headerColor, position: 'relative' }}>
          <span className="Home-ghost">{TITLE}</span>
          <span className="Home-typed">
            {titleText}
            {!titleDone && <span className="Home-cursor" style={{ color: theme.headerColor }}>|</span>}
          </span>
        </div>

        <div className="Home-divider" style={{ backgroundColor: theme.buttonColor }} />

        <div className="Home-description" style={{ color: theme.textColor, position: 'relative' }}>
          <span className="Home-ghost">{DESC}</span>
          <span className="Home-typed">
            {descText}
            {titleDone && !descDone && <span className="Home-cursor" style={{ color: theme.textColor }}>|</span>}
          </span>
        </div>
        ...
        <nav className="Home-nav-list">
          {navItems.map(item => (
            ...
            >
              <span className="Home-nav-icon" style={{ color: theme.buttonColor }}>
                {item.icon}
              </span>
              <span className="Home-nav-label">{item.label}</span>
              {item.external && (
                <span className="Home-nav-external" style={{ color: theme.buttonColor }}>
          ...
          <div className="tilt-float" ref={floatRef}>
            ...
            <div className="tilt-card" ref={cardRef}>
              ...
              <img
                ...
                style={{ filter: isDarkTheme ? 'invert(1)' : 'invert(0)', pointerEvents: 'none' }}
              />
            </div>
          </div>
        </div>
